...
int main(){
	...
	char rootOutputFileName1[50] = "InvarientMassHist_WIso15.root"; //Name of an output root file 
	...
	double MuonIsoBound = 1.5; //Upper bound to muon iso when looking for WMuons 
	...
	for(int i = 0; i < NEntries1; i++){
		...
		if(NMuon == 2){
			...
			if((Muon_Charge[0] == 1 && Muon_Charge[1] == -1) || (Muon_Charge[0] == -1 && Muon_Charge[1] == 1)){
				...
				histMuonIso->Fill(Muon_Iso_pos/pos_Muon_prime.Pt()); ///pos_Muon_prime.Pt());
				histMuonIso->Fill(Muon_Iso_neg/neg_Muon_prime.Pt()); ///neg_Muon_prime.Pt());
	...
	TCanvas *c1 = new TCanvas("c1","InvarientMass Histrogram" ,200,10,900,700);
		...
		if(FitInvarientMassHist){
			...
			c1_2->cd();
			histM_WMuon->SetXTitle("Invarient Mass");
