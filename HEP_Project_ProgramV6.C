...
int main(){
	...
	TH1F *histWMuonM = new TH1F("histWMuonM", "WMuon Mass hist", xNChannels, xMin, xMax);
	TH1F *histWPosMuonM = new TH1F("histWposMuonM", "WMuon Pos Mass hist", xNChannels, xMin, xMax);
	TH1F *histWNegMuonM = new TH1F("histWnegMuonM", "WMuon Neg Mass hist", xNChannels, xMin, xMax);
			...
			for(int j = 0; j < NMuon; j++){
				if(Muon_Iso[j] < MuonIsoBound_ForW){
					...
					if(!UseAllMuonsForW && (Muon_Charge[j] > 0)){
						cout<<" i: "<<i<<" j: "<<j<<" Muon_Charge: "<<Muon_Charge[j]<<endl;
						WMuon_prime.SetPxPyPzE(Muon_Px[j], Muon_Py[j], Muon_Pz[j], Muon_E[j]);
						//histWPosMuonM->Fill(WMuon_prime.M());
					}
					if(!UseAllMuonsForW && (Muon_Charge[j] < 0)){
						cout<<" i: "<<i<<" j: "<<j<<" Muon_Charge: "<<Muon_Charge[j]<<endl;
						WMuon_prime.SetPxPyPzE(Muon_Px[j], Muon_Py[j], Muon_Pz[j], Muon_E[j]);
						//histWNegMuonM->Fill(WMuon_prime.M());
					}
				}
			}
