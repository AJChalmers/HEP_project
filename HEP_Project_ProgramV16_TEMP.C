...
int main(){
	...
	char rootOutputFileName1[50] = "Soltuion_InvarientMassHist.root"; //Name of an output root file 
	char rootOutputFileName2[50] = "Soltuion_PosVNegPrime.root"; //Name of an output root file
	char rootOutputFileName3[50] = "Soltuion_PosVNeg.root"; //Name of an output root file
	char rootOutputFileName4[50] = "Soltuion_MuonIso_btag.root"; //Name of an output root file
	char rootOutputFileName5[50] = "Soltuion_MET_WMuonMass.root";
	char rootOutputFileName6[50] = "Soltuion_TransverseMassHist.root";
	char rootOutputFileName7[50] = "Soltuion_InvarientMassHist_TOP.root";
	char rootOutputFileName8[50] = "Soltuion_InvarientMassHist_W.root";
	char rootOutputFileName9[50] = "Soltuion_InvarientMassHist_Z.root";
	...
	double RelIsoBound = 0.05; //Iso/Pt bound aka MuonRelIsoCut in MyAnalysis.C
	double Jet_btag_Bound = 1.8; //Lower bound to Jet_btag
	double MuonPtCut = 25.;
	...
	TH1F *histM_Z = new TH1F("histM_Z", "Invarient Mass hist: Z (Solution)", 60 , 60, 120);
	...
	for(int i = 0; i < NEntries1; i++){
		...
		if((NIsoMuon > 1) && triggerIsoMu24 && (Muon_prime_Iso1.Pt() > MuonPtCut)){
			Z_prime = Muon_prime_Iso1 + Muon_prime_Iso2;
			histM_Z->Fill(tot_Muon_prime.M());
		}	
		...
		if(IsSelected){
			...
			for(int jet = 0; jet < NJet; jet++){  
				if(Jet_ID[jet] && (Jet_btag[jet] > Jet_btag_Bound)){
					...
					bJet_Pos.SetPxPyPzE(Jet_Px[jet], Jet_Py[jet], Jet_Pz[jet], Jet_E[jet]); 
	...
	TCanvas *c9 = new TCanvas("c9","InvarientMass Histrogram" ,200,10,900,700);
		c9->Divide(1,2);
		c9->SetFillColor(10);
		c9->SetGrid();
