...
//V11: Completely changed how WMuon loop works. Now collecting tops correctly?
///Note: This still isnt making Top dist correctly, also W has no distibution

//V12: V11 but Nicks version will all declarations up top 
...
int main(){
	...
	double MuonIsoBound_ForZ = 0.05; //Upper bound to muon iso when looking for ZMuons 
	double MuonIsoBound_ForW = 0.05; //Upper bound to muon iso when looking for WMuons 
	double ElectronIsoBound_ForW = 0.05; //Upper bound to electron iso when looking for WElectrons 
	...
	NEntries1 = NEntries1; //Cut for testing
	...
	for(int i = 0; i < NEntries1; i++){
		...
		for(int jet = 0; jet < NJet; jet++){  // Does the Muon loop for every jet
			if(Jet_ID[jet] && (Jet_btag[jet] > Jet_btag_Bound)){
				//For the WMuon and Top//
				if(NMuon > 1){
					cout<<"NMuon: "<<NMuon<<endl;
					histNMuon->Fill(NMuon);
					for(int j = 0; j < NMuon; j++){ // Muon loop: Looks at every loop
						if(Muon_Iso[j] < MuonIsoBound_ForW){
							...
							if(Muon_Charge[j] > 0){
								...
								bJet_Pos.SetPxPyPzE(Jet_Px[j], Jet_Py[j], Jet_Pz[j], Jet_E[j]); 
								
								double mW = 80.4;
								double A = pow(WMuon_prime_Pos.E(), 2) - pow(WMuon_prime_Pos.Pz(), 2);
								double B = mW * mW / 2. + (WMuon_prime_Pos.Px() * MET_px + WMuon_prime_Pos.Py() * MET_py);
								double D = pow(WMuon_prime_Pos.E(), 2) * (pow(B, 2) - pow(MET.Pt(), 2) * A);
								
								// first solution
								if(D >= 0){
								  double MET_pz = WMuon_prime_Pos.Pz() * B / A + sqrt(D) / A;
								  double MET_E = sqrt(MET_px * MET_px + MET_py * MET_py + MET_pz * MET_pz);
								  Neutrino1_Pos.SetPxPyPzE(MET_px, MET_py, MET_pz, MET_E);
								  WPos = Neutrino1_Pos + WMuon_prime_Pos;
								  histM_WPos->Fill(WPos.M());
								  histM_W->Fill(WPos.M());
								  
								  TopPos = WPos + bJet_Pos;
							...
							if(Muon_Charge[j] < 0){
								...
								MET.SetXYZM(MET_px, MET_py, 0., 0.);
								bJet_Neg.SetPxPyPzE(Jet_Px[j], Jet_Py[j], Jet_Pz[j], Jet_E[j]);
								
								double mW = 80.4;
								double A = pow(WMuon_prime_Neg.E(), 2) - pow(WMuon_prime_Neg.Pz(), 2);
								double B = mW * mW / 2. + (WMuon_prime_Neg.Px() * MET_px + WMuon_prime_Neg.Py() * MET_py);
								double D = pow(WMuon_prime_Neg.E(), 2) * (pow(B, 2) - pow(MET.Pt(), 2) * A);
								
								// first solution
								if(D >= 0){
								  double MET_pz = WMuon_prime_Neg.Pz() * B / A + sqrt(D) / A;
								  double MET_E = sqrt(MET_px * MET_px + MET_py * MET_py + MET_pz * MET_pz);
								  Neutrino1_Neg.SetPxPyPzE(MET_px, MET_py, MET_pz, MET_E);
								  WNeg = Neutrino1_Neg + WMuon_prime_Neg;
								  histM_WNeg->Fill(WNeg.M());
								  histM_W->Fill(WNeg.M());
								  
								  TopNeg = WNeg + bJet_Neg;
								  histM_TopNeg->Fill(TopNeg.M());
								  histM_Top->Fill(TopNeg.M());
								}
								// second solution
								if(D > 0){
								  double pz =  WMuon_prime_Neg.Pz() * B / A - sqrt(D) / A;
								  double E = sqrt(MET_px * MET_px + MET_py * MET_py + MET_pz * MET_pz);
								  Neutrino2_Neg.SetPxPyPzE(MET_px, MET_py, MET_pz, MET_E);
								  WNeg = Neutrino2_Neg + WMuon_prime_Neg;
								  histM_WNeg->Fill(WNeg.M());
								  histM_W->Fill(WNeg.M());
					...
					// if(((Muon_Charge[0] == 1 && Muon_Charge[1] == -1) || (Muon_Charge[0] == -1 && Muon_Charge[1] == 1)) && (Muon_Iso[j] < MuonIsoBound_ForW) && (Jet_ID[j] && (Jet_btag[j] > Jet_btag_Bound))){
						// for(int j = 0; j < NMuon; j++){
							// WPosHappened = true;
							// WNegHappened = true;
							
							// NWMuon = NWMuon + 1;
							// hist_WMuonVIso->Fill(Muon_Iso[j], NWMuon);
